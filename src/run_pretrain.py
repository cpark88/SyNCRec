...
#     parser.add_argument('--expert_disentangled', default='n', type=str, help='expert_disentangled or not')

#     parser.add_argument('--cross_attention', default='n', type=str, help='cross_attention or not')
#     parser.add_argument('--num_cross_layers', default=1, type=int, help='num_cross_layers or not')
    ...
    # torch.autograd.set_detect_anomaly(True) #debugging (사용 시 속도 느려짐)
    ...
    dist.barrier()
    # if args.local_rank==args.local_rank:
    if int(os.environ.get("LOCAL_RANK", 0)) == 0:
